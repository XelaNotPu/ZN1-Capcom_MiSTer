...
`ifdef DB9_LEAN
// Fit-constrained variants: the OSD autodetect probe FSM is compiled out
// (constant-0 probe_active prunes the FSM, its window counters and the
// probe-mode pin arms). Explicit joy_type selection - the actual routing
// path - is unaffected; only the OSD's pad-type auto-display is lost.
wire probe_active = 1'b0;
`else
...
// Saturn support is compiled out with the key gate on fit-constrained variants:
// the decoder ties inactive, so the probe FSM never latches saturn_active and
// the whole Saturn datapath prunes. DB9 MD / DB15 are unaffected.
assign JOYDBSATURN_1_VALID = 1'b0;
assign JOYDBSATURN_2_VALID = 1'b0;
assign JOYDBSATURN_1       = 16'h0000;
assign JOYDBSATURN_2       = 16'h0000;
`else
